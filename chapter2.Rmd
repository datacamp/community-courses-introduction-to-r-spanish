...
En nuestro camino a para quebrar el casino, haremos uso extensivo de los vectores. Un vector es un arreglo unidimensional que puede contener datos numéricos, caracteres, o valores lógicos. En otras palabras un vector es una herramienta simple para guardar datos. Por ejemplo podemos llevar la cuenta de las ganancias y pérdidas en los juegos de casino.

En R, se crea un vector con la función [`c()`](http://www.rdocumentation.org/packages/base/functions/c). La función se llama c por "Concatenar". Se ponen los valores o elementos del vector dentro de paréntesis, separados por coma. Por ejemplo:
...
*** =instructions
Ahora asigna los días de la semana como nombres a los vectores `vector_poker` y `vector_ruleta`. Usa los nombres con mayúscula inicial: Lunes, Martes, Miercoles, Jueves y Viernes.

*** =hint
Puedes usar `names(vector_poker)` para asignar nombres a los elementos de `vector_poker`. No olvides ni las mayúsculas ni las comillas al cread el vector de nombres:
```
...
---
## Dando nombres a un vector (2)

Si quieres ser un buen estadístico, tienes que ser un poco perezoso. (Si ya eres perezoso, quizás has nacido con esos excepcionales talentos estadísticos.)

En los ejercicios anteriores probablemente sentiste que es aburrido escribir una y otra vez información como los días de la semana. Sin embargo, cuando lo vemos desde otra perspectiva, existe una manera mas eficiente de lograr esto, que tal si asignamos el vector que contiene los días de la semana a una **variable**.
...
test_object("vector_total",
            undefined_msg = "Asegúrate de definir la variable <code>total_vector</code>.",
...
test_object("total_diario", 
            incorrect_msg = "Asegúrate de asignar la suma de <code>vector_poker</code> y <code>vector_ruleta</code> a <code>total_diario</code>.")
...
La función que nos ayudará a contestar esta pregunta es [`sum()`](http://www.rdocumentation.org/packages/base/functions/sum). La mencionada función calcula la suma de todos los elementos de un vector. Por ejemplo para calcular el monto total de dinero que has ganado/perdido en el poker en la semana, escribe: 
