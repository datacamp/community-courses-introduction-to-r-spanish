...
*** =hint
Usa la función [`list()`](http://www.rdocumentation.org/packages/base/functions/list) con `mi_vector`, `mi_matriz` and `mi_df` como argumentos separados por coma.
...
Cuando se crean listas es buena idea dar nombres a cada uno de los componentes de la misma para recordar fácilmente a que se refiere cada uno de los elementos. Es por ello buena práctica siempre dar nombres a los elementos de nuestras listas: 
...
success_msg("Excelente! No solo sabes construir listas sino también nombrar sus elementos, es una práctica muy útil y recomendable. Continuemos.")
```
