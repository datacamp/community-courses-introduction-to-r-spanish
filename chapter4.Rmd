...
Para crear factores en R usamos la función [`factor()`](http://www.rdocumentation.org/packages/base/functions/factor). Lo primero que tenemos que hacer es crear un vector que contenga todas las observaciones de nuestra variable.

First thing that you have to do is create a vector that contains all the observations that belong to a limited number of categories. For example, `gender_vector` contains the sex of 5 different individuals: 

```
gender_vector <- c("Masculino","Femenino","Femenino","Masculino","Masculino")
```

It is clear that there are two categories, or in R-terms **'factor levels'**, at work here: "Masculino" and "Femenino". 
...
Next, when you want to start your data analysis, your main concern is to keep a nice overview of all the variables and what they mean. At that point, you will often want to change the factor levels to `"Masculino"` and `"Femenino"` instead of `"M"` and `"F"` to make your life easier. 

*** =instructions
- Convert the character vector `survey_vector` into a factor vector. Assign it to `factor_survey_vector`.
- Change the factor levels of `factor_survey_vector` to `"Masculino"` and `"Femenino"`.
...
# Specify the levels of 'factor_survey_vector'
levels(factor_survey_vector) <- c("Femenino", "Masculino")
...
Going back to our survey, you would like to know how many `"Masculino"` responses you have in your study, and how many `"Femenino"` responses. The [`summary()`](http://www.rdocumentation.org/packages/base/functions/summary) function gives you the answer to this question.
...
success_msg("Nice! Have a look at the output. The fact that you identified <code>\"Masculino\"</code> and <code>\"Femenino\"</code> as factor levels in <code>factor_survey_vector</code> enables R to show the number of elements for each category.")
```
...
In `factor_survey_vector` we have a factor with two levels: Masculino and Femenino. But how does R value these relatively to each other? In other words, who does R think is better, males or females?
...
# Battle of the sexes: Masculino 'larger' than Femenino?
...
Since `"Masculino"` and `"Femenino"` are unordered (or nominal) factor levels, R returns a warning message, telling you that the greater than operator is not meaningful. As seen before, R attaches an equal value to the levels for such factors. 
